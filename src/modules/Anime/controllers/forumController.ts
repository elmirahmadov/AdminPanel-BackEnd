...
// Konu arama (yeni servis ile)
export async function searchTopics(req: Request, res: Response) {
  try {
    const topics = await topicService.searchTopics(req.query);
    res.json(topics);
  } catch (err) {
    res.status(500).json({
      error: "Konu arama yapılamadı.",
      detail: err instanceof Error ? err.message : err,
    });
  }
}

// ===== YENİ YANIT YÖNETİMİ FONKSİYONLARI =====

// Yanıt oluştur (yeni servis ile)
export async function createReplyWithService(req: Request, res: Response) {
  try {
    ...
    const reply = await replyService.createReply(Number(topicId), data);
...
// Yanıt güncelle (yeni servis ile)
export async function updateReplyWithService(req: Request, res: Response) {
  try {
    const { id } = req.params;
    const data = {
      ...req.body,
      editedBy: (req as any).user?.username || "Anonim",
    };

    const reply = await replyService.updateReply(Number(id), data);
    res.json(reply);
  } catch (err) {
    res.status(500).json({
      error: "Yanıt güncellenemedi.",
      detail: err instanceof Error ? err.message : err,
    });
  }
}

// Yanıt sil (yeni servis ile)
export async function deleteReplyWithService(req: Request, res: Response) {
  try {
    const { id } = req.params;
    await replyService.deleteReply(Number(id));
    res.json({ message: "Yanıt silindi." });
  } catch (err) {
    res.status(500).json({
      error: "Yanıt silinemedi.",
      detail: err instanceof Error ? err.message : err,
    });
  }
}
...
export async function likeReply(req: Request, res: Response) {
  try {
    ...
    await replyService.likeReply(Number(id), userId);
    res.json({ message: "Yanıt beğenildi." });
  } catch (err) {
    res.status(500).json({
...
// Yanıt beğenmeme
export async function dislikeReply(req: Request, res: Response) {
  try {
    const { id } = req.params;
    const userId = (req as any).user?.userId || 1;
    await replyService.dislikeReply(Number(id), userId);
    res.json({ message: "Yanıt beğenilmedi." });
  } catch (err) {
    res.status(500).json({
      error: "Yanıt beğenilemedi.",
      detail: err instanceof Error ? err.message : err,
...
// ===== MODERASYON FONKSİYONLARI =====

// Konu moderasyonu
export async function moderateTopic(req: Request, res: Response) {
  try {
    const { id } = req.params;
    const { action, note } = req.body;
    const moderatorId = (req as any).user?.userId || 1;

    await moderationService.moderateTopic(
      Number(id),
      action,
      moderatorId,
      note
    );
    res.json({ message: "Konu moderasyonu tamamlandı." });
  } catch (err) {
    res.status(500).json({
      error: "Konu moderasyonu yapılamadı.",
      detail: err instanceof Error ? err.message : err,
    });
  }
}
...
// Yanıt moderasyonu
export async function moderateReply(req: Request, res: Response) {
  try {
    const { id } = req.params;
    const { action, note } = req.body;
    const moderatorId = (req as any).user?.userId || 1;

    await moderationService.moderateReply(
      Number(id),
      action,
      moderatorId,
      note
    );
    res.json({ message: "Yanıt moderasyonu tamamlandı." });
  } catch (err) {
    res.status(500).json({
      error: "Yanıt moderasyonu yapılamadı.",
      detail: err instanceof Error ? err.message : err,
    });
  }
}
...
export async function unbanUser(req: Request, res: Response) {
  try {
    const { forumId, userId } = req.params;
    await moderationService.unbanUser(forumId, Number(userId));
...
export async function addModerator(req: Request, res: Response) {
  try {
    const { forumId, userId } = req.params;
    const grantedBy = (req as any).user?.username || "Admin";

    await moderationService.addModerator(forumId, Number(userId), grantedBy);
    res.json({ message: "Moderatör eklendi." });
  } catch (err) {
    res.status(500).json({
      error: "Moderatör eklenemedi.",
      detail: err instanceof Error ? err.message : err,
    });
  }
}
...
export async function removeModerator(req: Request, res: Response) {
  try {
    const { forumId, userId } = req.params;
    await moderationService.removeModerator(forumId, Number(userId));
    res.json({ message: "Moderatör çıkarıldı." });
  } catch (err) {
    res.status(500).json({
      error: "Moderatör çıkarılamadı.",
      detail: err instanceof Error ? err.message : err,
    });
  }
}
...
export async function getModerationQueue(req: Request, res: Response) {
  try {
    const { forumId, status } = req.query;
    ...
    res.status(500).json({
      error: "Moderasyon kuyruğu getirilemedi.",
      detail: err instanceof Error ? err.message : err,
    });
  }
}
...
export async function createForumCategory(req: Request, res: Response) {
...
export async function updateForumCategory(req: Request, res: Response) {
  const { id } = req.params;
  const { name, description, order } = req.body;
  try {
...
export async function deleteForumCategory(req: Request, res: Response) {
  const { id } = req.params;
  try {
    await prisma.forumCategory.delete({ where: { id: Number(id) } });
...
export async function getForumTopicById(req: Request, res: Response) {
  const { id } = req.params;
  try {
